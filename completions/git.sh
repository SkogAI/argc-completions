...
# @option --s <--sshhoorrtt>                Give the output in the short-format.
# @option --- <--sshhoorrtt>                Give the output in the short-format.
# @option --b <--bbrraanncchh>             Show the branch and tracking info even in short-format.
# @option --- <--bbrraanncchh>             Show the branch and tracking info even in short-format.
# @option --- <--sshhooww--ssttaasshh>  Show the number of entries currently stashed away.
# @option --- <--ppoorrcceellaaiinn[==_<_v_e_r_s_i_o_n_>]>  Give the output in an easy-to-parse format for scripts.
# @option --- <--lloonngg>                   Give the output in the long-format.
# @option --v <--vveerrbboossee>          In addition to the names of files that have been changed, also show the textual changes that are staged to be committed (i.e., like the output of ggiitt ddiiffff ----ccaacchheedd).
# @option --- <--vveerrbboossee>          In addition to the names of files that have been changed, also show the textual changes that are staged to be committed (i.e., like the output of ggiitt ddiiffff ----ccaacchheedd).
...
# @option --- <--iiggnnoorree--ssuubbmmoodduulleess[==_<_w_h_e_n_>]>  Ignore changes to submodules when looking for changes.
# @option --- <--iiggnnoorreedd[==_<_m_o_d_e_>]>  Show ignored files as well.
# @option --z <zz>                               Terminate entries with _N_U_L, instead of _L_F.
# @option --- <--nnoo--ccoolluummnn>    Display untracked files in columns.
# @option --- <--nnoo--aahheeaadd--bbeehhiinndd>  Display or do not display detailed ahead/behind counts for the branch relative to its upstream branch.
# @option --- <--nnoo--rreennaammeess>  Turn on/off rename detection regardless of user configuration.
# @option --- <--ffiinndd--rreennaammeess[==_<_n_>]>  Turn on rename detection, optionally setting the similarity threshold.
status() {
    :;
}
# }} git status
...
# @option --z <--nnuullll>                   When showing sshhoorrtt or ppoorrcceellaaiinn ggiitt--ssttaattuuss(1) output, print the filename verbatim and terminate the entries with _N_U_L, instead of _L_F.
# @option --- <--nnuullll>                   When showing sshhoorrtt or ppoorrcceellaaiinn ggiitt--ssttaattuuss(1) output, print the filename verbatim and terminate the entries with _N_U_L, instead of _L_F.
...
# @option --- <--nnoo--aallll>             Fetch all remotes, except for the ones that has the rreemmoottee.._<_n_a_m_e_>..sskkiippFFeettcchhAAllll configuration variable set.
# @option --a <--aappppeenndd>             Append ref names and object names of fetched refs to the existing contents of ..ggiitt//FFEETTCCHH__HHEEAADD.
# @option --- <--aappppeenndd>             Append ref names and object names of fetched refs to the existing contents of ..ggiitt//FFEETTCCHH__HHEEAADD.
# @option --- <--aattoommiicc>             Use an atomic transaction to update local refs.
# @option --- <--ddeepptthh==_<_d_e_p_t_h_>>  Limit fetching to the specified number of commits from the tip of each remote branch history.
# @option --- <--ddeeeeppeenn==_<_d_e_p_t_h_>>  Similar to ----ddeepptthh, except it specifies the number of commits from the current shallow boundary instead of from the tip of each remote branch history.
# @option --- <--sshhaallllooww--ssiinnccee==_<_d_a_t_e_>>  Deepen or shorten the history of a shallow repository to include all reachable commits after _<_d_a_t_e_>.
# @option --- <--sshhaallllooww--eexxcclluuddee==_<_r_e_f_>>  Deepen or shorten the history of a shallow repository to exclude commits reachable from a specified remote branch or tag.
# @option --- <--uunnsshhaallllooww>    If the source repository is complete, convert a shallow repository to a complete one, removing all the limitations imposed by shallow repositories.
# @option --- <--uuppddaattee--sshhaallllooww>  By default when fetching from a shallow repository, ggiitt ffeettcchh refuses refs that require updating ..ggiitt//sshhaallllooww.
# @option --- <--nneeggoottiiaattiioonn--ttiipp==(_<_c_o_m_m_i_t_>|_<_g_l_o_b_>)>  By default, Git will report, to the server, commits reachable from all local refs to find common commits in an attempt to reduce the size of the to-be-received packfile.
# @option --- <--nneeggoottiiaattee--oonnllyy>  Do not fetch anything from the server, and instead print the ancestors of the provided ----nneeggoottiiaattiioonn--ttiipp== arguments, which we have in common with the server.
# @option --- <--ddrryy--rruunn>          Show what would be done, without making any changes.
# @option --- <--ppoorrcceellaaiinn>    Print the output to standard output in an easy-to-parse format for scripts.
# @option --- <--nnoo--wwrriittee--ffeettcchh--hheeaadd>  Write the list of remote refs fetched in the FFEETTCCHH__HHEEAADD file directly under $$GGIITT__DDIIRR.
# @option --f <--ffoorrccee>                When ggiitt ffeettcchh is used with _<_s_r_c_>::_<_d_s_t_> refspec, it may refuse to update the local branch as discussed in the _<_r_e_f_s_p_e_c_> part below.
# @option --- <--ffoorrccee>                When ggiitt ffeettcchh is used with _<_s_r_c_>::_<_d_s_t_> refspec, it may refuse to update the local branch as discussed in the _<_r_e_f_s_p_e_c_> part below.
# @option --k <--kkeeeepp>                   Keep downloaded pack.
# @option --- <--kkeeeepp>                   Keep downloaded pack.
# @option --- <--mmuullttiippllee>       Allow several _<_r_e_p_o_s_i_t_o_r_y_> and _<_g_r_o_u_p_> arguments to be specified.
# @option --- <--nnoo--aauuttoo--ggcc>  Run ggiitt mmaaiinntteennaannccee rruunn ----aauuttoo at the end to perform automatic repository maintenance if needed.
# @option --- <--nnoo--wwrriittee--ccoommmmiitt--ggrraapphh>  Write a commit-graph after fetching.
# @option --- <--pprreeffeettcchh>       Modify the configured refspec to place all refs into the rreeffss//pprreeffeettcchh// namespace.
# @option --p <--pprruunnee>                Before fetching, remove any remote-tracking references that no longer exist on the remote.
# @option --- <--pprruunnee>                Before fetching, remove any remote-tracking references that no longer exist on the remote.
# @option --P <--pprruunnee--ttaaggss>  Before fetching, remove any local tags that no longer exist on the remote if ----pprruunnee is enabled.
# @option --- <--pprruunnee--ttaaggss>  Before fetching, remove any local tags that no longer exist on the remote if ----pprruunnee is enabled.
# @option --n <--nnoo--ttaaggss>          By default, tags that point at objects that are downloaded from the remote repository are fetched and stored locally.
# @option --- <--nnoo--ttaaggss>          By default, tags that point at objects that are downloaded from the remote repository are fetched and stored locally.
# @option --- <--rreeffeettcchh>          Instead of negotiating with the server to avoid transferring commits and associated objects that are already present locally, this option fetches all objects as a fresh clone would.
# @option --- <--rreeffmmaapp==_<_r_e_f_s_p_e_c_>>  When fetching refs listed on the command line, use the specified refspec (can be given more than once) to map the refs to remote-tracking branches, instead of the values of rreemmoottee.._<_n_a_m_e_>..ffeettcchh configuration variables for the remote repository.
# @option --t <--ttaaggss>                   Fetch all tags from the remote (i.e., fetch remote tags rreeffss//ttaaggss//** into local tags with the same name), in addition to whatever else would otherwise be fetched.
# @option --- <--ttaaggss>                   Fetch all tags from the remote (i.e., fetch remote tags rreeffss//ttaaggss//** into local tags with the same name), in addition to whatever else would otherwise be fetched.
# @option --- <--rreeccuurrssee--ssuubbmmoodduulleess[==(yyeess|oonn--ddeemmaanndd|nnoo)]>  Control if and under what conditions new commits of submodules should be fetched too.
# @option --j <--jjoobbss==_<_n_>>       Parallelize all forms of fetching up to _<_n_> jobs at a time.
# @option --- <--jjoobbss==_<_n_>>       Parallelize all forms of fetching up to _<_n_> jobs at a time.
# @option --- <--nnoo--rreeccuurrssee--ssuubbmmoodduulleess>  Disable recursive fetching of submodules (this has the same effect as using the ----rreeccuurrssee--ssuubbmmoodduulleess==nnoo option).
# @option --- <--sseett--uuppssttrreeaamm>  If the remote is fetched successfully, add upstream (tracking) reference, used by argument-less ggiitt--ppuullll(1) and other commands.
# @option --- <--ssuubbmmoodduullee--pprreeffiixx==_<_p_a_t_h_>>  Prepend _<_p_a_t_h_> to paths printed in informative messages such as "Fetching submodule foo".
# @option --- <--rreeccuurrssee--ssuubbmmoodduulleess--ddeeffaauulltt==(yyeess|oonn--ddeemmaanndd)>  This option is used internally to temporarily provide a non-negative default value for the ----rreeccuurrssee--ssuubbmmoodduulleess option.
# @option --u <--uuppddaattee--hheeaadd--ookk>  By default ggiitt ffeettcchh refuses to update the head which corresponds to the current branch.
# @option --- <--uuppddaattee--hheeaadd--ookk>  By default ggiitt ffeettcchh refuses to update the head which corresponds to the current branch.
# @option --- <--uuppllooaadd--ppaacckk> <_<_u_p_l_o_a_d_-_p_a_c_k_>>  When given, and the repository to fetch from is handled by ggiitt ffeettcchh--ppaacckk, ----eexxeecc==_<_u_p_l_o_a_d_-_p_a_c_k_> is passed to the command to specify non-default path for the command run on the other end.
# @option --q <--qquuiieett>                Pass ----qquuiieett to ggiitt--ffeettcchh--ppaacckk and silence any other internally used git commands.
# @option --- <--qquuiieett>                Pass ----qquuiieett to ggiitt--ffeettcchh--ppaacckk and silence any other internally used git commands.
# @option --v <--vveerrbboossee>          Be verbose.
# @option --- <--vveerrbboossee>          Be verbose.
# @option --- <--pprrooggrreessss>       Progress status is reported on the standard error stream by default when it is attached to a terminal, unless --qq is specified.
# @option --o <--sseerrvveerr--ooppttiioonn==_<_o_p_t_i_o_n_>>  Transmit the given string to the server when communicating using protocol version 2.
# @option --- <--sseerrvveerr--ooppttiioonn==_<_o_p_t_i_o_n_>>  Transmit the given string to the server when communicating using protocol version 2.
# @option --- <--sshhooww--ffoorrcceedd--uuppddaatteess>  By default, git checks if a branch is force-updated during fetch.
# @option --- <--nnoo--sshhooww--ffoorrcceedd--uuppddaatteess>  By default, git checks if a branch is force-updated during fetch.
# @option --4 <--iippvv44>                   Use IPv4 addresses only, ignoring IPv6 addresses.
# @option --- <--iippvv44>                   Use IPv4 addresses only, ignoring IPv6 addresses.
# @option --6 <--iippvv66>                   Use IPv6 addresses only, ignoring IPv4 addresses.
# @option --- <--iippvv66>                   Use IPv6 addresses only, ignoring IPv4 addresses.
# @option --- <--ssttddiinn>                Read refspecs, one per line, from stdin in addition to those provided as arguments.
...
# @option --q <--qquuiieett>                This is passed to both underlying git-fetch to squelch reporting of during transfer, and underlying git-merge to squelch output during merging.
# @option --- <--qquuiieett>                This is passed to both underlying git-fetch to squelch reporting of during transfer, and underlying git-merge to squelch output during merging.
# @option --v <--vveerrbboossee>          Pass ----vveerrbboossee to git-fetch and git-merge.
# @option --- <--vveerrbboossee>          Pass ----vveerrbboossee to git-fetch and git-merge.
# @option --- <--nnoo--rreeccuurrssee--ssuubbmmoodduulleess>  This option controls if new commits of populated submodules should be fetched, and if the working trees of active submodules should be updated, too (see ggiitt--ffeettcchh(1), ggiitt--ccoonnffiigg(1) and ggiittmmoodduulleess(5)).
# @option --- <--nnoo--ccoommmmiitt>    Perform the merge and commit the result.
# @option --- <--nnoo--eeddiitt>          Invoke an editor before committing successful mechanical merge to further edit the auto-generated merge message, so that the user can explain and justify the merge.
# @option --e <--nnoo--eeddiitt>          Invoke an editor before committing successful mechanical merge to further edit the auto-generated merge message, so that the user can explain and justify the merge.
# @option --- <--cclleeaannuupp==_<_m_o_d_e_>>  This option determines how the merge message will be cleaned up before committing.
...
# @option --- <--nnoo--aauuttoossttaasshh>  Automatically create a temporary stash entry before the operation begins, record it in the ref MMEERRGGEE__AAUUTTOOSSTTAASSHH and apply it after the operation ends.
# @option --- <--aallllooww--uunnrreellaatteedd--hhiissttoorriieess>  By default, ggiitt mmeerrggee command refuses to merge histories that do not share a common ancestor.
# @option --r <--rreebbaassee[==(ttrruuee|mmeerrggeess|ffaallssee|iinntteerraaccttiivvee)]>  ttrruuee rebase the current branch on top of the upstream branch after fetching.
# @option --- <--rreebbaassee[==(ttrruuee|mmeerrggeess|ffaallssee|iinntteerraaccttiivvee)]>  ttrruuee rebase the current branch on top of the upstream branch after fetching.
# @option --- <--nnoo--rreebbaassee>    This is shorthand for ----rreebbaassee==ffaallssee.
# @option --- <--nnoo--aallll>             Fetch all remotes, except for the ones that has the rreemmoottee.._<_n_a_m_e_>..sskkiippFFeettcchhAAllll configuration variable set.
# @option --a <--aappppeenndd>             Append ref names and object names of fetched refs to the existing contents of ..ggiitt//FFEETTCCHH__HHEEAADD.
# @option --- <--aappppeenndd>             Append ref names and object names of fetched refs to the existing contents of ..ggiitt//FFEETTCCHH__HHEEAADD.
# @option --- <--aattoommiicc>             Use an atomic transaction to update local refs.
...
# @option --- <--ddrryy--rruunn>          Show what would be done, without making any changes.
# @option --- <--ppoorrcceellaaiinn>    Print the output to standard output in an easy-to-parse format for scripts.
# @option --f <--ffoorrccee>                When ggiitt ffeettcchh is used with _<_s_r_c_>::_<_d_s_t_> refspec, it may refuse to update the local branch as discussed in the _<_r_e_f_s_p_e_c_> part of the ggiitt--ffeettcchh(1) documentation.
# @option --- <--ffoorrccee>                When ggiitt ffeettcchh is used with _<_s_r_c_>::_<_d_s_t_> refspec, it may refuse to update the local branch as discussed in the _<_r_e_f_s_p_e_c_> part of the ggiitt--ffeettcchh(1) documentation.
# @option --k <--kkeeeepp>                   Keep downloaded pack.
# @option --- <--kkeeeepp>                   Keep downloaded pack.
# @option --- <--pprreeffeettcchh>       Modify the configured refspec to place all refs into the rreeffss//pprreeffeettcchh// namespace.
# @option --p <--pprruunnee>                Before fetching, remove any remote-tracking references that no longer exist on the remote.
...
# @option --- <--bbrraanncchheess>       Push all branches (i.e.
# @option --- <--pprruunnee>                Remove remote branches that don’t have a local counterpart.
# @option --- <--mmiirrrroorr>             Instead of naming each ref to push, specifies that all refs under rreeffss// (which includes but is not limited to rreeffss//hheeaaddss//, rreeffss//rreemmootteess//, and rreeffss//ttaaggss//) be mirrored to the remote repository.
# @option --n <--ddrryy--rruunn>          Do everything except actually send the updates.
# @option --- <--ddrryy--rruunn>          Do everything except actually send the updates.
# @option --- <--ppoorrcceellaaiinn>    Produce machine-readable output.
# @option --d <--ddeelleettee>             All listed refs are deleted from the remote repository.
# @option --- <--ddeelleettee>             All listed refs are deleted from the remote repository.
# @option --- <--ttaaggss>                   All refs under rreeffss//ttaaggss are pushed, in addition to refspecs explicitly listed on the command line.
# @option --- <--ffoollllooww--ttaaggss>  Push all the refs that would be pushed without this option, and also push annotated tags in rreeffss//ttaaggss that are missing from the remote but are pointing at commit-ish that are reachable from the refs being pushed.
# @option --- <--ssiiggnneedd==(ttrruuee|ffaallssee|iiff--aasskkeedd)>  GPG-sign the push request to update refs on the receiving side, to allow it to be checked by the hooks and/or be logged.
# @option --- <--nnoo--aattoommiicc>    Use an atomic transaction on the remote side if available.
# @option --o <--ppuusshh--ooppttiioonn==_<_o_p_t_i_o_n_>>  Transmit the given string to the server, which passes them to the pre-receive as well as the post-receive hook.
# @option --- <--ppuusshh--ooppttiioonn==_<_o_p_t_i_o_n_>>  Transmit the given string to the server, which passes them to the pre-receive as well as the post-receive hook.
# @option --- <--eexxeecc==_<_g_i_t_-_r_e_c_e_i_v_e_-_p_a_c_k_>>  Path to the _g_i_t_-_r_e_c_e_i_v_e_-_p_a_c_k program on the remote end.
# @option --- <--nnoo--ffoorrccee--wwiitthh--lleeaassee>
# @option --- <--ffoorrccee--wwiitthh--lleeaassee==_<_r_e_f_n_a_m_e_>::_<_e_x_p_e_c_t_>>  Usually, ggiitt ppuusshh refuses to update a remote ref that is not an ancestor of the local ref used to overwrite it.
# @option --f <--ffoorrccee>                Usually, ggiitt ppuusshh will refuse to update a branch that is not an ancestor of the commit being pushed.
# @option --- <--ffoorrccee>                Usually, ggiitt ppuusshh will refuse to update a branch that is not an ancestor of the commit being pushed.
# @option --- <--nnoo--ffoorrccee--iiff--iinncclluuddeess>  Force an update only if the tip of the remote-tracking ref has been integrated locally.
# @option --- <--rreeppoo==_<_r_e_p_o_s_i_t_o_r_y_>>  This option is equivalent to the _<_r_e_p_o_s_i_t_o_r_y_> argument.
# @option --u <--sseett--uuppssttrreeaamm>  For every branch that is up to date or successfully pushed, add upstream (tracking) reference, used by argument-less ggiitt--ppuullll(1) and other commands.
# @option --- <--sseett--uuppssttrreeaamm>  For every branch that is up to date or successfully pushed, add upstream (tracking) reference, used by argument-less ggiitt--ppuullll(1) and other commands.
# @option --- <--nnoo--tthhiinn>          These options are passed to ggiitt--sseenndd--ppaacckk(1).
# @option --q <--qquuiieett>                Suppress all output, including the listing of updated refs, unless an error occurs.
# @option --- <--qquuiieett>                Suppress all output, including the listing of updated refs, unless an error occurs.
# @option --v <--vveerrbboossee>          Run verbosely.
# @option --- <--vveerrbboossee>          Run verbosely.
# @option --- <--pprrooggrreessss>       Progress status is reported on the standard error stream by default when it is attached to a terminal, unless --qq is specified.
# @option --- <--rreeccuurrssee--ssuubbmmoodduulleess==(cchheecckk|oonn--ddeemmaanndd|oonnllyy|nnoo)>  May be used to make sure all submodule commits used by the revisions to be pushed are available on a remote-tracking branch.
# @option --- <--nnoo--vveerriiffyy>    Toggle the pre-push hook (see ggiitthhooookkss(5)).
# @option --4 <--iippvv44>                   Use IPv4 addresses only, ignoring IPv6 addresses.
# @option --- <--iippvv44>                   Use IPv4 addresses only, ignoring IPv6 addresses.
# @option --6 <--iippvv66>                   Use IPv6 addresses only, ignoring IPv4 addresses.
# @option --- <--iippvv66>                   Use IPv6 addresses only, ignoring IPv4 addresses.
...
# @option --b <bb>                               Show blank SHA-1 for boundary commits.
# @option --- <--rroooott>                   Do not treat root commits as boundaries.
# @option --- <--sshhooww--ssttaattss>  Include additional statistics at the end of blame output.
# @option --L <LL> <::_<_f_u_n_c_n_a_m_e_>>  Annotate only the line range given by _<_s_t_a_r_t_>,,_<_e_n_d_>, or by the function name regex _<_f_u_n_c_n_a_m_e_>.
# @option --l <ll>                               Show long rev (Default: off).
# @option --t <tt>                               Show raw timestamp (Default: off).
# @option --S <SS> <_<_r_e_v_s_-_f_i_l_e_>>  Use revisions from _<_r_e_v_s_-_f_i_l_e_> instead of calling ggiitt--rreevv--lliisstt(1).
# @option --- <--rreevveerrssee> <_<_s_t_a_r_t_>...._<_e_n_d_>>  Walk history forward instead of backward.
# @option --- <--ffiirrsstt--ppaarreenntt>  Follow only the first parent commit upon seeing a merge commit.
# @option --p <--ppoorrcceellaaiinn>    Show in a format designed for machine consumption.
# @option --- <--ppoorrcceellaaiinn>    Show in a format designed for machine consumption.
# @option --- <--lliinnee--ppoorrcceellaaiinn>  Show the porcelain format, but output commit information for each line, not just the first time a commit is referenced.
# @option --- <--iinnccrreemmeennttaall>  Show the result incrementally in a format designed for machine consumption.
# @option --- <--eennccooddiinngg==_<_e_n_c_o_d_i_n_g_>>  Specify the encoding used to output author names and commit summaries.
# @option --- <--ccoonntteennttss> <_<_f_i_l_e_>>  Annotate using the contents from _<_f_i_l_e_>, starting from _<_r_e_v_> if it is specified, and HHEEAADD otherwise.
# @option --- <--ddaattee> <_<_f_o_r_m_a_t_>>  Specify the format used to output dates.
# @option --- <--nnoo--pprrooggrreessss>  Enable progress reporting on the standard error stream even if not attached to a terminal.
# @option --M <MM[_<_n_u_m_>]>              Detect moved or copied lines within a file.
# @option --C <CC[_<_n_u_m_>]>              In addition to --MM, detect lines moved or copied from other files that were modified in the same commit.
# @option --- <--iiggnnoorree--rreevv> <_<_r_e_v_>>  Ignore changes made by the revision when assigning blame, as if the change never happened.
# @option --- <--iiggnnoorree--rreevvss--ffiillee> <_<_f_i_l_e_>>  Ignore revisions listed in _<_f_i_l_e_>, which must be in the same format as an ffsscckk..sskkiippLLiisstt.
# @option --- <--ccoolloorr--lliinneess>  Color line annotations in the default format differently if they come from the same commit as the preceding line.
# @option --- <--ccoolloorr--bbyy--aaggee>  Color line annotations depending on the age of the line in the default format.
# @option --h <hh>                               Show help message.
# @option --c <cc>                               Use the same output mode as ggiitt--aannnnoottaattee(1) (Default: off).
# @option --- <--ssccoorree--ddeebbuugg>  Include debugging information related to the movement of lines between files (see --CC) and lines moved within a file (see --MM).
# @option --f <--sshhooww--nnaammee>    Show the filename in the original commit.
# @option --- <--sshhooww--nnaammee>    Show the filename in the original commit.
# @option --n <--sshhooww--nnuummbbeerr>  Show the line number in the original commit (Default: off).
# @option --- <--sshhooww--nnuummbbeerr>  Show the line number in the original commit (Default: off).
# @option --s <ss>                               Suppress the author name and timestamp from the output.
# @option --e <--sshhooww--eemmaaiill>  Show the author email instead of the author name (Default: off).
# @option --- <--sshhooww--eemmaaiill>  Show the author email instead of the author name (Default: off).
# @option --w <ww>                               Ignore whitespace when comparing the parent’s version and the child’s to find where the lines came from.
# @option --- <--ddiiffff--aallggoorriitthhmm==(ppaattiieennccee|mmiinniimmaall|hhiissttooggrraamm|mmyyeerrss)>  Choose a diff algorithm.
# @option --- <--aabbbbrreevv==_<_n_>>  Instead of using the default _7_+_1 hexadecimal digits as the abbreviated object name, use _<_m_>_+_1 digits, where _<_m_> is at least _<_n_> but ensures the commit object names are unique.
blame() {
    :;
}
# }} git blame
...
# @option --get-urlmatch <name> <URL>           Replaced by ggiitt ccoonnffiigg ggeett ----uurrll==_<_U_R_L_> _<_n_a_m_e_>.
...
# @option --blob <blob-id>        read config from given blob object
# @option --no-blob <blob-id>     read config from given blob object
# @flag --all                     unset all multi-valued config options
# @flag --no-all                  unset all multi-valued config options
# @option --value <pattern>       unset multi-valued config options with matching values
# @option --no-value <pattern>    unset multi-valued config options with matching values
# @flag --fixed-value             use string equality when comparing values to value pattern
# @flag --no-fixed-value          use string equality when comparing values to value pattern
# @arg name!
config::unset() {
...
# @option --max-cruft-size <n>                Override ----mmaaxx--ppaacckk--ssiizzee for cruft packs.
# @option --combine-cruft-below-size <n>      When generating cruft packs without pruning, only repack existing cruft packs whose size is strictly less than _<_n_> bytes, which can optionally be suffixed with "k", "m", or "g".
