...
# @flag --molecule                        string  Associated molecule ID
# @option --notify <string[="mayor/"]>    Additional address to notify on completion (default: mayor/ if flag used without value)
# @flag --owner                           string  Owner who requested convoy (gets completion notification)
# @arg name!
# @arg issues*
convoy::create() {
    :;
...
# @flag --notify     string  Agent to notify on close (e.g., mayor/)
# @flag --reason     string  Reason for closing the convoy
# @arg convoy-id!
convoy::close() {
    :;
}
...
# @flag --cleanup-status    string  Git cleanup status: clean, uncommitted, unpushed, stash, unknown (ZFC: agent-observed)
# @flag --gate              string  Gate bead ID to wait on (with --phase-complete)
# @flag -h --help           help for done
# @flag --issue             string  Source issue ID (default: parse from branch name)
# @flag --phase-complete    Signal phase complete - await gate before continuing
# @flag -p --priority       int  Override priority (0-4, default: inherit from issue) (default -1)
# @flag --status            string  Exit status: COMPLETED, ESCALATED, or DEFERRED (default "COMPLETED")
done_() {
    :;
}
# }} gt done
...
# @flag --pr                int  GitHub PR number to run formula on
...
# @flag -m --message                          string   Message body for handoff mail (optional)
# @flag -s --subject                          string   Subject for handoff mail (optional)
...
# @flag -m --message    string   Message for handoff mail (optional)
# @flag -s --subject    string   Subject for handoff mail (optional)
# @arg bead-id
hook() {
    :;
...
# @flag --timeout                    string  Maximum time to wait for signal (e.g., 30s, 5m) (default "60s")
...
# @flag --branch     string  Override branch name template (supports {epic}, {prefix}, {user})
# @flag -h --help    help for create
# @arg epic-id!
...
# @flag --strategy    string  Ordering strategy: 'priority' or 'fifo' (default "priority")
# @arg rig!
mq::next() {
    :;
}
...
# @flag -r --reason    string  Reason for rejection (required)
# @arg rig!
# @arg mr-id-or-branch!
mq::reject() {
    :;
...
# @flag --branch         string  Source branch (default: current branch)
# @flag --epic           string  Target epic's integration branch instead of main
...
# @flag -m --message    string   Context notes for resumption
# @arg gate-id!
park() {
    :;
}
...
# @flag -r --reason    string  Reason for releasing (added as note)
# @arg issue-id+
release() {
    :;
}
...
# @flag --account       string  Claude Code account handle to use
# @flag --agent         string  Override agent/runtime for this sling (e.g., claude, gemini, codex, or custom alias)
# @flag -a --args       string  Natural language instructions for the executor (e.g., 'patch release')
...
# @flag -m --message    string  Context message for the work
# @flag --no-convoy     Skip auto-convoy creation for single-issue sling
# @flag --on            string  Apply formula to existing bead (implies wisp scaffolding)
# @flag -s --subject    string  Context subject for the work
# @flag --var           stringArray  Formula variable (key=value), can be repeated
...
# @flag --rig                     string  Target rig for synthesis polecat
# @arg convoy-id!
synthesis::start() {
    :;
}
...
# @flag --del        stringArray  Delete label (repeatable)
# @flag -h --help    help for state
# @flag --incr       string  Increment numeric label (creates with value 1 if missing)
# @flag --json       Output as JSON
# @flag --set        stringArray  Set label value (format: key=value, repeatable)
# @flag -a --all     Include polecats in the menu
# @arg agent-bead!
agents::state() {
    :;
...
# @flag --reason         string  Reason for force-kill (included in notifications)
...
# @flag --failures                 int  Number of consecutive failures before recommending force-kill (default 3)
...
# @flag --reason     string  Reason for pausing the Deacon
deacon::pause() {
    :;
}
# }}} gt deacon pause
...
# @flag --dog           string  Dispatch to specific dog (default: any idle)
# @flag -n --dry-run    Show what would be done without doing it
# @flag -h --help       help for dispatch
# @flag --json          Output as JSON
# @flag --plugin        string  Plugin name to dispatch (required)
# @flag --rig           string  Limit plugin search to specific rig
# @arg name!
dog::dispatch() {
    :;
}
...
# @flag --json         Output as JSON
# @flag --threshold    int  Commits behind main to consider stale (default 20)
# @arg rig!
polecat::stale() {
    :;
}
...
# @flag --polecat    string   Polecat/crew member name
# @flag --rig        string   Rig name (required for rig-specific roles)
# @arg role
role::home() {
    :;
}
...
# @flag -f --file       string   File to read message from
# @flag -h --help       help for inject
# @flag -m --message    string   Message to inject
# @arg rig-polecat <<rig>/<polecat>>
session::inject() {
    :;
}
...
# @flag --agent      string  Agent alias to run the Witness with (overrides town default)
# @flag --env        stringArray  Environment variable override (KEY=VALUE, can be repeated)
# @flag -h --help    help for restart
# @arg rig!
witness::restart() {
    :;
...
# @flag --json           Output as JSON
# @flag -r --reason      string  Detailed reason for escalation
# @flag --related        string  Related bead ID (task, bug, etc.)
# @flag -s --severity    string  Severity level: critical, high, medium, low (default "medium")
# @flag --source         string  Source identifier (e.g., plugin:rebuild-gt, patrol:deacon)
# @arg description
escalate() {
    :;
}
...
# @flag -h --help    help for close
# @flag --reason     string  Resolution reason
# @arg escalation-id!
escalate::close() {
    :;
}
...
# {{{ gt mail inbox
# @cmd View your inbox
# @flag --address      string  Alias for --identity
# @flag -h --help      help for inbox
# @flag --identity     string  Explicit identity for inbox (e.g., greenplace/Toast)
# @flag --json         Output as JSON
...
# {{{ gt mail send
# @cmd Send a message
# @flag --cc                           stringArray  CC recipients (can be used multiple times)
# @flag -h --help                      help for send
# @flag -m --message                   string  Message body
...
# @flag --priority                     int  Message priority (0=urgent, 1=high, 2=normal, 3=low, 4=backlog) (default 2)
# @flag --reply-to                     string  Message ID this is replying to
# @flag --self                         Send to self (auto-detect from cwd)
# @flag -s --subject                   string  Message subject (required)
...
# @flag --claimers    string  Pattern for who can claim from this queue (required)
# @flag -h --help     help for create
# @arg name!
mail::queue::create() {
    :;
...
# @flag -m --message    string   Reply message body (required)
# @flag -s --subject    string   Override reply subject (default: Re: <original>)
# @arg message-id!
mail::reply() {
    :;
}
...
# @flag -m --message    string   Message to send
# @arg target!
# @arg message
nudge() {
    :;
...
# @flag -w --wait          int  Seconds to wait for graceful shutdown (default 30) (default 30)
...
# @flag --account    string   Claude Code account handle to use
# @flag --agent      string   Agent alias to run crew worker with (overrides rig/town default)
# @flag -h --help    help for crew
# @flag --rig        string   Rig to use
# @arg name!
start::crew() {
    :;
}
...
# @flag --account        string   Claude Code account handle to use (overrides default)
# @flag --agent          string   Agent alias to run crew worker with (overrides rig/town default)
...
# @flag -m --message    string   Custom handoff message
# @flag --rig           string   Rig to use
# @arg name!
crew::refresh() {
    :;
...
# @flag --github     string  Create GitHub repo (format: owner/repo, private by default)
...
# @flag -n --name                   string  Town name (defaults to directory name)
# @flag --no-beads                  Skip town beads initialization
# @flag --owner                     string  Owner email for entity identity (defaults to git config user.email)
...
# @flag --branch                   string  Default branch name (default: auto-detected from remote)
# @flag -h --help                  help for add
# @option --local-repo <string>    Local repo path to share git objects (optional)
# @flag --prefix                   string  Beads issue prefix (default: derived from name)
# @arg name!
# @arg git-url!
rig::add() {
    :;
...
# @flag --actor                           string   Actor emitting the event (auto-detected if not set)
# @flag --count                           int   Polecat count (for patrol events)
# @flag -h --help                         help for emit
# @flag --issue                           string   Issue ID (for polecat_checked)
# @flag --message                         string   Human-readable message
# @flag --polecat                         string   Polecat involved (for polecat_checked, polecat_nudged)
# @flag --reason                          string   Reason for the action
# @flag --rig                             string   Rig the event is about
# @option --status[working|idle|stuck]    string   Status
# @flag --target                          string   Target of the action (for escalation)
...
# @flag --molecule    string  Override molecule ID (auto-detected if not specified)
# @flag --notes       string  Add notes to the checkpoint
# @flag --step        string  Override step ID (auto-detected if not specified)
checkpoint::write() {
    :;
}
# }}} gt checkpoint write
...
# @flag --session                 string  Tmux session name to record
...
# @flag --rig                 string  Check specific rig only
...
# @flag --agent        string   Agent ID (e.g., greenplace/Toast)
# @flag --exit-code    int   Exit code from pane (default -1)
# @flag -h --help      help for crash
# @flag --session      string   Tmux session name
log::crash() {
    :;
}
# }}} gt log crash
...
# @flag --json         Output as JSON
# @flag -p --prompt    string  One-shot prompt (with --talk)
# @flag -n --recent    int  Number of recent sessions to show (default 20)
# @flag --rig          string  Filter by rig name
# @flag --role         string  Filter by role (crew, polecat, witness, etc.)
# @flag -t --talk      string  Session ID to commune with
seance() {
    :;
}
# }} gt seance
...
# @flag -h --help    help for next
# @flag --session    string   Override current session (used by tmux binding)
cycle::next() {
    :;
}
# }}} gt cycle next
