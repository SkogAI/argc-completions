...
# @option --cdi-spec-dir* <dir>         Set the CDI spec directory path (may be set multiple times) (default [/etc/cdi,/var/run/cdi])
# @option --cgroup-manager[cgroupfs|systemd] <string>  Cgroup manager to use (default "systemd")
# @option --config <path>               Path to directory containing authentication config file
# @option --conmon <path>               Path of the conmon binary
# @option -c --connection <string>      Connection to use for remote Podman service (CONTAINER_CONNECTION) (default "podman-machine-default")
...
# @option --identity <file>             path to SSH identity file, (CONTAINER_SSHKEY) (default "/home/skogix/.local/share/containers/podman/machine/machine")
...
# @flag --syslog                        Output podman-internal logs to syslog as well as the console (default false)
# @option --tls-ca <file>               path to TLS certificate Authority PEM file for remote.
# @option --tls-cert <file>             path to TLS client certificate PEM file for remote.
# @option --tls-details <file>          Path to a containers-tls-details.yaml(5) file
# @option --tls-key <file>              path to TLS client certificate private key PEM file for remote.
...
# @option --url <string>                URL to access Podman service (CONTAINER_HOST) (default "ssh://claude@127.0.0.1:43071/run/user/1000/podman/podman.sock")
# @flag -v --version                    version for podman
# @option --volumepath <dir>            Path to the volume directory in which volume data is stored

# {{ docker artifact
# @cmd Manage OCI artifacts
artifact() {
    :;
}

# {{{ docker artifact add
# @cmd Add an OCI artifact to the local store
# @option --annotation <annotation>    set an annotation for the specified files of artifact
# @flag -a --append                    Append files to an existing artifact
# @option --file-type <file>           Set file type to use for the artifact (layer)
# @flag --replace                      Replace an existing artifact
# @option --type <string>              Use type to describe an artifact
# @arg artifact
# @arg path*
artifact::add() {
    :;
...
# {{{ docker artifact extract
# @cmd Extract an OCI artifact to a local path
# @option --digest <string>    Only extract blob with the given digest
# @option --title <string>     Only extract blob with the given title
# @arg artifact
# @arg path
artifact::extract() {
    :;
...
# {{{ docker artifact inspect
# @cmd Inspect an OCI artifact
# @option -f --format <string>    Format volume output using JSON or a Go template (default "json")
# @arg artifact
artifact::inspect() {
    :;
}
...
# {{{ docker artifact ls
# @cmd List OCI artifacts
# @option --format <string>    Format volume output using JSON or a Go template (default "{{range .}}{{.Repository}}\t{{.Tag}}\t{{.Digest}}\t{{.Created}}\t{{.Size}}\n{{end -}}")
...
# {{{ docker artifact pull
# @cmd Pull an OCI artifact
...
# {{{ docker artifact push
# @cmd Push an OCI artifact
# @option --authfile <file>                        Path of the authentication file.
...
# @option --sign-by-sq-fingerprint <FINGERPRINT>   Sign the image using a Sequoia-PGP key with the specified FINGERPRINT
...
# {{{ docker artifact rm
# @cmd Remove one or more OCI artifacts
# @flag -a --all       Remove all artifacts
# @flag -i --ignore    Ignore error if artifact does not exist
# @arg artifact*
artifact::rm() {
    :;
}
...
# @option --compression-format <string>       compression format to use for layers and cache
# @option --compression-level <int>           compression level to use for layers and cache
...
# @flag --created-annotation                  set an "org.opencontainers.image.created" annotation in the image (default true)
...
# @option --iidfile-raw <file>                file to write the image ID to (without algorithm prefix)
# @flag --inherit-annotations                 inherit the annotations from the base image or base stages.
# @flag --inherit-labels                      inherit the labels from the base image or base stages.
...
# @option --metadata-file <file>              file to write metadata about the image to
# @option --mount* <string>                   set transient mounts for each RUN instruction, e.g. type=secret,id=mysecret
# @option --network <path>                    'private', 'none', 'ns:path' of network namespace to join, or 'host'
...
# @option --platform[`_module_oci_docker_platform`] <OS/ARCH[/VARIANT]>  set the OS/ARCH[/VARIANT] of the image to the provided value instead of the current operating system and architecture of the host (for example "linux/arm") (default [linux/amd64])
# @option --pull[always|missing|never|newer] <string[="always"]>  Pull image policy (default "missing")
...
# @flag --rewrite-timestamp                   set timestamps in layers to no later than the value for --source-date-epoch
# @flag --rm                                  remove intermediate containers after a successful build (default true)
# @option --runtime-flag* <string>            add global flags for the container runtime
# @flag --save-stages                         save intermediate stage images.
...
# @option --source-date-epoch <seconds>       set new timestamps in image info to seconds after the epoch, defaults to current time
# @option --source-policy-file <pathname>     pathname of source policy file for controlling source references during build
...
# @flag --stage-labels                        add metadata labels to intermediate stage images (requires --save-stages).
...
# @option --timestamp <seconds>               set new timestamps in image info and layer to seconds after the epoch, defaults to current times
...
# @option --unsetannotation* <string>         unset annotation when inheriting annotations from base image
...
# @option --health-interval <string>               set an interval for the healthcheck.
# @option --health-log-destination <path>          set the destination of the HealthCheck log.
# @option --health-max-log-count <uint>            set maximum number of attempts in the HealthCheck log file.
# @option --health-max-log-size <uint>             set maximum length in characters of stored HealthCheck log.
...
# @option --health-startup-interval <string>       Set an interval for the startup healthcheck.
...
# @option --health-timeout <string>                the maximum time allowed to complete the healthcheck before an interval is considered failed and SIGKILL is sent to the healthcheck process (default "30s")
# @flag --help
# @option -h --hostname <string>                   Set container hostname
# @option --hosts-file[image|none] <file>          Base file to create the /etc/hosts file inside the container, or one of the special values.
...
# @flag -i --interactive                           Make STDIN available to the contained process
...
# @option --name <string>                          Assign a name to the container
# @option --network* <string>                      Connect a container to a network
...
# @flag --no-hostname                              Do not create /etc/hostname within the container, instead use the version from the image
...
# @option --platform[`_module_oci_docker_platform`] <string>  Specify the platform for selecting the image.
# @option --pod <string>                           Run container in an existing pod
...
# @option --cidfile <file>          File to read the container ID from
...
# @flag -i --interactive            Make STDIN available to the contained process
# @flag -l --latest                 Act on the latest container podman is aware of Not supported with the "--remote" flag
# @flag --no-session                Do not create a database session for the exec process
...
# @option -p --publish* <string>        Publish a container's port, or a range of ports, to the host (default [])
# @flag --tcp-close                     Restore a container and close all TCP connections
...
# @flag --exit-first-match          Wait for exit of first container which matches conditions, ignore other ones
...
# @option -f --filter*[`_choice_event_filter`] <string>  filter output
...
# @option --sort <choice>          Sort by created, id, repository, size, tag (default created)
# @arg image[`_module_oci_docker_image`]
image::list() {
    :;
}
...
# @flag --build-cache           Remove persistent build cache created by --mount=type=cache
...
# @option --os                        Use OS instead of the running OS for choosing images
# @option --platform[`_module_oci_docker_platform`] <string>  Specify the platform for selecting the image.
# @option --policy[always|missing|never|newer] <string>  Pull image policy (default "always")
# @flag -q --quiet                    Suppress output information when pulling images
# @option --retry <uint>              number of times to retry in case of failure when performing pull (default 3)
# @option --retry-delay <string>      delay between retries in case of pull failures
# @flag --tls-verify                  Require HTTPS and verify certificates when contacting registries (default true)
...
# @option --format[oci-archive|docker-archive]    podman save  Format for podman save when creating the transfer archive.
# @flag -q --quiet                                Suppress the output
# @arg image[`_module_oci_docker_image`]
# @arg host <HOST::>
image::scp() {
...
# @option --signature-policy <file>    Path to a signature-policy file
...
# @flag -a --all               Show all images (default hides intermediate images)
# @flag --digests              Show digests
# @option -f --filter*[`_choice_image_ls_filter`] <string>  Filter output based on conditions provided (default [])
# @option --format <string>    Change the output format to JSON or a Go template
# @flag --history              Display the image name history
# @flag --no-trunc             Do not truncate output
# @flag -n --noheading         Do not print column headings
...
# @flag -l --latest               Act on the latest container podman is aware of Not supported with the "--remote" flag
# @flag -s --size                 Display total file size
# @option -t --type <string>      Specify inspect-object type (default "all")
# @arg enum[ARTIFACT|CONTAINER|IMAGE|POD|NETWORK|VOLUME]
inspect() {
    :;
}
# }} docker inspect
...
# @arg kubefile-kubefile <[KUBEFILE [KUBEFILE...]]|->
kube::down() {
    :;
}
# }}} docker kube down

# {{{ docker kube generate
# @cmd Generate Kubernetes YAML from containers, pods or volumes.
# @option -f --filename <file>     Write output to the specified path
# @flag --podman-only              Add podman-only reserved annotations to the generated YAML file (Cannot be used by Kubernetes)
# @option -r --replicas <int32>    Set the replicas number for Deployment kind (default 1)
...
# @option --annotation* <string>                   Add Podman-specific annotations to containers and pods created by Podman (key=value)
# @option --authfile <file>                        Path of the authentication file.
...
# @option --network* <string>                      Connect pod to network(s) or network mode
# @flag --no-hostname                              Do not create /etc/hostname within the container, instead use the version from the image
# @flag --no-hosts                                 Do not create /etc/hosts within the pod's containers, instead use the version from the image
# @flag --no-pod-prefix                            Do not prefix container name with pod name
...
# {{{ docker machine cp
# @cmd Securely copy contents between the virtual machine
# @flag -q --quiet    Suppress copy status output
# @arg src_path
# @arg dest_path
machine::cp() {
    :;
...
# @option --image <string>          Bootable image for machine (default "docker://quay.io/podman/machine-os")
# @flag --import-native-ca          Import the host trusted CA certificates into the machine
# @option -m --memory <uint>        Memory in MiB (default 2048)
# @flag --now                       Start machine now
# @option --playbook <string>       Run an Ansible playbook after first boot
# @option --provider <string>       Override the default machine provider
# @flag --rootful                   Whether this machine should prefer rootful container execution
# @option -s --swap <uint>          Swap in MiB
# @option --timezone <string>       Set timezone (default "local")
# @flag --tls-verify                Require HTTPS and verify certificates when contacting registries (default true)
# @flag -u --update-connection      Set default system connection for this machine
...
# @option -v --volume* <string>     Volumes to mount, source:target (default [$HOME:$HOME,/home/skogix/.config/containers:/etc/containers])
# @arg name
machine::init() {
    :;
}
...
# {{{{ docker machine os upgrade
# @cmd Upgrade machine os
# @flag -n --dry-run              Only check if an upgrade is available
# @option -f --format <string>    suppress output except for specified format.
# @flag --restart                 Restart VM to upgrade
# @arg name
machine::os::upgrade() {
    :;
}
...
# @cmd Remove an item from a manifest list or image index
# @arg list
# @arg digest
manifest::remove() {
    :;
...
# @flag -i --ignore    Ignore errors when a specified manifest is missing
# @arg list*
manifest::rm() {
    :;
}
...
# @option --route* <string>            Static routes for this network.
...
# @flag -a --all               Show all the containers, default is only running containers
# @flag --external             Show containers in storage not controlled by Podman
# @option -f --filter*[`_choice_container_ls_filter`] <string>  Filter output based on conditions given
# @option --format <string>    Pretty-print containers to JSON or using a Go template
# @option -n --last <int>      Print the n last created containers (all states) (default -1)
# @flag -l --latest            Act on the latest container podman is aware of Not supported with the "--remote" flag
# @flag --no-trunc             Display the extended information
...
# {{ docker quadlet
# @cmd Allows users to manage Quadlets
quadlet() {
    :;
}

# {{{ docker quadlet install
# @cmd Install a quadlet file or quadlet application
# @option --application <dir>    Group quadlets and associated file in a directory named after the application
# @flag --reload-systemd         Reload systemd after installing Quadlets (default true)
# @flag -r --replace             Replace the installation even if the quadlet already exists
# @arg quadlet-path-or-url
# @arg files-path-or-url*
quadlet::install() {
    :;
}
# }}} docker quadlet install
...
# @cmd List Quadlets
# @option -f --filter* <string>    Filter output based on conditions given
# @option --format <path>          Pretty-print output to JSON or using a Go template (default "{{range .}}{{.Name}}\t{{.UnitName}}\t{{.Path}}\t{{.Status}}\t{{.App}}\t{{.Pod}}\n{{end -}}")
# @flag -n --noheading             Do not print headers
quadlet::list() {
    :;
}
...
# {{{ docker quadlet print
# @cmd Display the contents of a quadlet
# @arg quadlet
quadlet::print() {
    :;
}
...
# {{{ docker quadlet rm
# @cmd Remove Quadlets
# @flag -a --all            Remove all Quadlets for the current user
# @flag -f --force          Remove running quadlets
# @flag -i --ignore         Do not error for Quadlets that do not exist
# @flag --recursive         Remove all Quadlets belonging to the specified application and its directory
# @flag --reload-systemd    Reload systemd after removal (default true)
# @arg quadlet-application* <QUADLET|APPLICATION>
quadlet::rm() {
    :;
}
# }}} docker quadlet rm
...
# @flag --ignore                            If a secret with the same name exists, ignore and do not create a new secret
...
# @option --format <string>        Format secret output using Go template (default "{{range .}}{{.ID}}\t{{.Name}}\t{{.Driver}}\t{{.CreatedAt}}\t{{.UpdatedAt}}\n{{end -}}")
# @flag -n --noheading             Do not print headers
...
# {{{ docker system connection
# @cmd Manage remote API service destinations
system::connection() {
    :;
}

# {{{{ docker system connection add
...
# @option --tls-ca <file>         path to TLS certificate Authority PEM file
# @option --tls-cert <file>       path to TLS client certificate PEM file
# @option --tls-key <file>        path to TLS client certificate private key PEM file
# @arg name
# @arg destination
...
# {{{{ docker system connection default
...
# {{{{ docker system connection remove
...
# {{{{ docker system connection rename
...
# {{{ docker system migrate
# @cmd Migrate containers
# @flag --migrate-db                Migrate database from BoltDB to SQLite
...
# @flag --build                 Remove build containers
...
# @option --tls-cert <file>         PEM file containing TLS serving certificate.
# @option --tls-client-ca <file>    Only trust client connections with certificates signed by this CA PEM file
# @option --tls-key <file>          PEM file containing TLS serving certificate private key
# @arg uri
system::service() {
    :;
}
...
# @flag --rootless-netns    Join the rootless network namespace used for netavark networking
# @arg command
# @arg arg*
unshare() {
    :;
...
# {{{ docker volume prune
# @cmd Remove unused volumes
# @flag -a --all                Remove all unused volumes, both anonymous and named
# @flag --dry-run               Show what would be pruned without actually pruning
# @option --filter* <string>    Provide filter values (e.g. 'label=<key>=<value>')
# @flag -f --force              Do not prompt for confirmation
volume::prune() {
    :;
