...
# @option -n --lines <[+]NUM>              output the last NUM lines, instead of the last 10; or use -n +NUM to skip NUM-1 lines at the start
...
# @option --pid[`_module_os_pid`]          with -f, terminate after process ID, PID dies; can be repeated to watch multiple processes
