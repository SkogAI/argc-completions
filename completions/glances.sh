...
# @option -P --plugins <PLUGIN_DIR>                path to additional plugin directory
...
# @option --disable <DISABLE_PLUGIN>               disable plugin (comma-separated list or all).
...
# @option --enable <ENABLE_PLUGIN>                 enable plugin (comma-separated list)
# @flag --disable-process                          disable process module
# @flag --disable-webui                            disable the Web Interface
# @flag --light                                    light mode for Curses UI (disable all but the top menu)
# @flag --enable-light                             light mode for Curses UI (disable all but the top menu)
# @flag -0 --disable-irix                          task's cpu usage will be divided by the total number of CPUs
# @flag -1                                         start Glances in per CPU mode
# @flag --percpu                                   start Glances in per CPU mode
# @flag --per-cpu                                  start Glances in per CPU mode
...
# @flag --disable-separator                        disable separator in the UI (between top and others modules)
...
# @flag -w --webserver                             run Glances in web server mode (FastAPI, Uvicorn, Jinja2 libs needed)
...
# @flag --hide-kernel-threads                      hide kernel threads in the process list (not available on Windows)
# @flag -b --byte                                  display network rate in bytes per second
...
# @flag --hide-public-info                         hide public information (like public IP)
