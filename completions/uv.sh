...
# @option --directory                    Change to the given directory prior to running the command
...
# @option --python-platform[windows|linux|macos|x86_64-pc-windows-msvc|aarch64-pc-windows-msvc|i686-pc-windows-msvc|x86_64-unknown-linux-gnu|aarch64-apple-darwin|x86_64-apple-darwin|aarch64-unknown-linux-gnu|aarch64-unknown-linux-musl|x86_64-unknown-linux-musl|x86_64-manylinux2014|x86_64-manylinux_2_17|x86_64-manylinux_2_28|x86_64-manylinux_2_31|x86_64-manylinux_2_32|x86_64-manylinux_2_33|x86_64-manylinux_2_34|x86_64-manylinux_2_35|x86_64-manylinux_2_36|x86_64-manylinux_2_37|x86_64-manylinux_2_38|x86_64-manylinux_2_39|x86_64-manylinux_2_40|aarch64-manylinux2014|aarch64-manylinux_2_17|aarch64-manylinux_2_28|aarch64-manylinux_2_31|aarch64-manylinux_2_32|aarch64-manylinux_2_33|aarch64-manylinux_2_34|aarch64-manylinux_2_35|aarch64-manylinux_2_36|aarch64-manylinux_2_37|aarch64-manylinux_2_38|aarch64-manylinux_2_39|aarch64-manylinux_2_40|wasm32-pyodide2024] <PYTHON_PLATFORM>  The platform for which requirements should be installed
...
# @option --with-requirements <WITH_REQUIREMENTS>  Include all requirements listed in the given `requirements.txt` files
...
# @option --output-format[text|json] <OUTPUT_FORMAT>  Select the output format [default: text]
# @option --python-downloads-json-url <PYTHON_DOWNLOADS_JSON_URL>  URL pointing to JSON of custom Python installations [env: UV_PYTHON_DOWNLOADS_JSON_URL=]
# @flag -n --no-cache                    Avoid reading from or writing to the cache, instead using a temporary directory for the duration of the operation [env: UV_NO_CACHE=]
# @option --cache-dir <CACHE_DIR>        Path to the cache directory [env: UV_CACHE_DIR=/mnt/extra/uv]
# @flag --managed-python                 Require use of uv-managed Python versions [env: UV_MANAGED_PYTHON=]
# @flag --no-managed-python              Disable use of uv-managed Python versions [env: UV_NO_MANAGED_PYTHON=]
...
# @option --mirror                          Set the URL to use as the source for downloading Python installations [env: UV_PYTHON_INSTALL_MIRROR=]
# @option --pypy-mirror <PYPY_MIRROR>       Set the URL to use as the source for downloading PyPy installations [env: UV_PYPY_INSTALL_MIRROR=]
# @option --python-downloads-json-url <PYTHON_DOWNLOADS_JSON_URL>  URL pointing to JSON of custom Python installations [env: UV_PYTHON_DOWNLOADS_JSON_URL=]
...
# @option -r --requirements                       Install all packages listed in the given `requirements.txt` or `pylock.toml` files
...
# @option -r --requirements              Uninstall all packages listed in the given requirements files
...
# @option --trusted-publishing[automatic|always|never] <TRUSTED_PUBLISHING>  Configure using trusted publishing through GitHub Actions
# @option --keyring-provider[disabled|subprocess] <KEYRING_PROVIDER>  Attempt to use `keyring` for authentication for remote requirements files [env: UV_KEYRING_PROVIDER=]
