...
# @option --alt-svc <filename>                     Enable alt-svc with this cache file
# @flag --anyauth                                  Pick any authentication method
# @flag -a --append                                Append to target file when uploading
# @option --aws-sigv4 <provider1[:prvdr2[:reg[:srv]]]>  AWS V4 signature auth
# @flag --basic                                    HTTP Basic Authentication
# @flag --ca-native                                Load CA certs from the OS
...
# @flag --digest                                   HTTP Digest Authentication
...
# @option --etag-save <file>                       Parse incoming ETag and save to a file
...
# @flag --ftp-pasv                                 Send PASV/EPSV instead of PORT
# @option -P --ftp-port <address>                  Send PORT instead of PASV
...
# @flag --ftp-ssl-control                          Require TLS for login, clear for transfer
# @flag -G --get                                   Put the post data in the URL and use GET
# @flag -g --globoff                               Disable URL globbing with {} and []
# @option --happy-eyeballs-timeout-ms <ms>         Time for IPv6 before IPv4
# @option --haproxy-clientip <ip>                  Set address in HAProxy PROXY
...
# @flag --mail-rcpt-allowfails                     Allow RCPT TO command to fail
...
# @flag --proxy-ca-native                          Load CA certs from the OS to verify proxy
# @option --proxy-cacert <file>                    CA certificates to verify proxy against
...
# @flag --proxy-tlsv1                              TLSv1 for HTTPS proxy
# @option -U --proxy-user <user:password>          Proxy user and password
# @option --proxy1.0 <host[:port]>                 Use HTTP/1.0 proxy on given port
# @flag -p --proxytunnel                           HTTP proxy tunnel (using CONNECT)
# @option --pubkey <key>                           SSH Public key filename
...
# @flag -O --remote-name                           Write output to file named as remote file
# @flag --remote-name-all                          Use the remote filename for all URLs
# @flag -R --remote-time                           Set remote file's time on local output
...
# @option --resolve <[+]host:port:addr[,addr]...>  Resolve host+port to address
# @option --retry <num>                            Retry request if transient problems occur
# @flag --retry-all-errors                         Retry all errors (with --retry)
# @flag --retry-connrefused                        Retry on connection refused (with --retry)
...
# @flag --socks5-basic                             Username/password auth for SOCKS5 proxies
...
# @option --socks5-hostname <host[:port]>          SOCKS5 proxy, pass hostname to proxy
...
# @flag --tcp-nodelay                              Set TCP_NODELAY
...
# @option --tlsuser <name>                         TLS username
# @flag -1 --tlsv1                                 TLSv1.0 or greater
# @flag --tlsv1.0                                  TLSv1.0 or greater
# @flag --tlsv1.1                                  TLSv1.1 or greater
# @flag --tlsv1.2                                  TLSv1.2 or greater
# @flag --tlsv1.3                                  TLSv1.3 or greater
...
# @flag --trace-ids                                Transfer + connection ids in verbose output
