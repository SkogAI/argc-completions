...
# @option -s --size <_s_i_z_e_-_i_n_d_e_x>  or --size _w_i_d_t_hx_h_e_i_g_h_t This sets the screen size, either matching by size or using the index into the list of available sizes.
