...
# @option --device-size <bytes>                Use only specified device size (ignore rest of device), DANGEROUS!
# @flag --disable-blkid                        Disable blkid on-disk signature detection and wiping
...
# @flag --dump-volume-key                      Dump volume key instead of keyslots info
# @flag --encrypt                              Encrypt LUKS2 device (in-place encryption)
# @option --external-tokens-path <STRING>      Path to directory with external token handlers (plugins).
# @flag --force-password                       Disable password quality check (if enabled)
# @flag --force-offline-reencrypt              Force offline LUKS2 reencryption and bypass active device detection
# @flag --force-no-keyslots                    Force dangerous reencryption operation erasing all remaining keyslots
...
# @flag --hw-opal                              Use HW OPAL encryption together with SW encryption
# @flag --hw-opal-factory-reset                Wipe WHOLE OPAL disk on luksErase
# @flag --hw-opal-only                         Use only HW OPAL encryption
...
# @flag --integrity-inline                     Use inline mode (use HW integrity field)
# @option --integrity-key-size <BITS>          The size of the data integrity key
...
# @flag --keep-key                             Do not change volume key
# @option --key-description <STRING>           Keyring key description
...
# @option --link-vk-to-keyring <STRING>        Set keyring where to link volume key
...
# @option --new-keyfile <STRING>               Read the key for a new slot from a file
...
# @option --new-key-description <STRING>       Keyring new key description
# @option --new-key-size <BITS>                The size of the new encryption key
# @option --new-key-slot <INT>                 Slot number for new key (default is first free)
# @option --new-token-id <INT>                 Token number (default: any)
# @option --new-volume-key-file <STRING>       Use the new volume key from file
# @option --new-volume-key-keyring <STRING>    Use the specified keyring key as new volume key
...
# @flag --perf-high_priority                   Set dm-crypt workqueues and the writer thread to high priority
...
# @flag --progress-json                        Print progress data in json format (suitable for machine processing)
# @option --progress-frequency <secs>          Progress line update (in seconds)
# @flag -r --readonly                          Create a readonly mapping
# @option --reduce-device-size <bytes>         Reduce data device size (move data offset), DANGEROUS!
...
# @flag --token-replace                        Replace the current token
...
# @flag --unbound                              Create or dump unbound LUKS2 keyslot (unassigned to data segment) or LUKS2 token (unassigned to keyslot)
...
# @option --volume-key-file <STRING>           Use the volume key from file
# @option --volume-key-keyring <STRING>        Use the specified keyring key as a volume key
# @option -B --block-size <MiB>                Reencryption block size
# @flag -N --new                               Create new header on not encrypted device
# @flag --use-directio                         Use direct-io when accessing devices
...
# @cmd dump FVAULT2 device information
# @meta inherit-flag-options
# @arg device![`_choice_device`]
fvault2Dump() {
    :;
