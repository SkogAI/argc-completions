...
# @flag -n --no-clobber                           (deprecated) silently skip existing files.
# @flag -P --no-dereference                       never follow symbolic links in SOURCE
# @flag -p                                        same as --preserve=mode,ownership,timestamps
# @option --preserve[`_choice_preserve`] <ATTR_LIST>  preserve the specified attributes
...
# @option --update                                control which existing files are updated; UPDATE={all,none,none-fail,older(default)}
# @flag -u                                        equivalent to --update[=older].
# @flag -v --verbose                              explain what is being done
# @flag --keep-directory-symlink                  follow existing symlinks to directories
